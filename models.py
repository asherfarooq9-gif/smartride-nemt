...
class Ride(Base):
    ...
    ride_type:           Mapped[RideType]             = mapped_column(SAEnum(RideType))
...
class TriageEvent(Base):
    ...
    predicted_specialty: Mapped[Specialty]   = mapped_column(SAEnum(Specialty))
    confidence_score:    Mapped[float]       = mapped_column(Numeric(5, 4))
    severity_level:      Mapped[SeverityLevel] = mapped_column(SAEnum(SeverityLevel))
...
class HospitalAlert(Base):
    ...
    channel:     Mapped[str]             = mapped_column(String(20))  # 'fhir' | 'sms'
