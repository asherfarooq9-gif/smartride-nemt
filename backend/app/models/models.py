...
class Driver(Base):
    ...
    wallet_balance_pkr: Mapped[float] = mapped_column(
        DOUBLE_PRECISION, default=0.0, server_default="0"
    )
...
class Ride(Base):
    ...
    ride_type: Mapped[RideType] = mapped_column(
        SAEnum(RideType, name="ride_type", create_type=False)
    )
    ...
    driver_assigned_at: Mapped[Optional[datetime]] = mapped_column(
    ...
    patient: Mapped["Patient"] = relationship(
        "Patient", back_populates="rides", lazy="noload"
    )
    driver: Mapped[Optional["Driver"]] = relationship(
        "Driver", back_populates="rides", lazy="noload"
    )
    hospital: Mapped[Optional["Hospital"]] = relationship(
        "Hospital", foreign_keys=[hospital_id], lazy="noload"
    )
    triage_events: Mapped[List["TriageEvent"]] = relationship(
        "TriageEvent", back_populates="ride"
    )
    hospital_alerts: Mapped[List["HospitalAlert"]] = relationship(
        "HospitalAlert", back_populates="ride"
    )
    family_notifications: Mapped[List["FamilyNotification"]] = relationship(
        "FamilyNotification", back_populates="ride"
    )
...
class TriageEvent(Base):
    ...
    predicted_specialty: Mapped[Specialty] = mapped_column(
    ...
    severity_level: Mapped[SeverityLevel] = mapped_column(
        SAEnum(SeverityLevel, name="severity_level", create_type=False)
    )
    ...
    __table_args__ = (Index("ix_triage_ride_id", "ride_id"),)
