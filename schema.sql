-- SmartRide NEMT — PostgreSQL Schema
-- Run: psql -U postgres -d smartride -f schema.sql
...
CREATE TYPE ride_type AS ENUM ('emergency', 'scheduled');
...
CREATE TABLE patients (
  ...
  date_of_birth         DATE,
  mobility_needs        TEXT,                        -- e.g. 'wheelchair', 'stretcher'
...
CREATE TABLE drivers (
  ...
  vehicle_type    VARCHAR(50) NOT NULL,              -- e.g. 'sedan', 'wheelchair_van'
...
CREATE TABLE hospitals (
  ...
  fhir_endpoint       TEXT,                          -- null = SMS fallback
...
CREATE TABLE rides (
  ...
  -- scheduling (null = emergency / immediate)
...
CREATE TABLE triage_events (
  ...
  rule_override       BOOLEAN DEFAULT FALSE,           -- true if keyword rule fired
  model_version       VARCHAR(50) NOT NULL,
  inference_ms        INTEGER,                         -- latency tracking
  created_at          TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_triage_ride ON triage_events(ride_id);
...
CREATE TABLE hospital_alerts (
  ...
  channel         VARCHAR(20) NOT NULL,               -- 'fhir' | 'sms'
  payload         JSONB,                              -- FHIR bundle or SMS body
  sent_at         TIMESTAMPTZ,
...
-- ─── ANALYTICS SNAPSHOTS (pre-aggregated, refreshed hourly) ──────────────────
...
-- ─── REFRESH TRIGGER: keep location synced with lat/lng ───────────────────────
...
-- ─── SEED: Islamabad hospitals (starter data) ─────────────────────────────────
...
('Pakistan Institute of Medical Sciences (PIMS)', 'G-8/3, Islamabad', 'Islamabad', 33.7005, 73.0679, '+92519255000',
  ARRAY['cardiology','neurology','orthopedics','obstetrics','pediatrics','oncology','nephrology','pulmonology','gastroenterology','general_emergency']::specialty[], 120, '+92519255001'),
('Shifa International Hospital', 'Sector H-8/4, Islamabad', 'Islamabad', 33.6715, 73.0566, '+92518463000',
  ARRAY['cardiology','neurology','orthopedics','obstetrics','pediatrics','oncology','nephrology','ophthalmology','ent','dermatology','general_emergency']::specialty[], 100, '+92518463001'),
('Poly Clinic Hospital', 'G-6/2, Islamabad', 'Islamabad', 33.7215, 73.0826, '+92519207500',
  ARRAY['cardiology','orthopedics','pediatrics','obstetrics','general_emergency']::specialty[], 60, '+92519207501'),
('Holy Family Hospital', 'Murree Road, Rawalpindi', 'Rawalpindi', 33.5811, 73.0479, '+92519290301',
  ARRAY['cardiology','neurology','orthopedics','obstetrics','pediatrics','psychiatry','general_emergency']::specialty[], 80, '+92519290302'),
('Benazir Bhutto Hospital', 'Murree Road, Rawalpindi', 'Rawalpindi', 33.5751, 73.0462, '+92519290401',
  ARRAY['cardiology','orthopedics','obstetrics','pediatrics','general_emergency']::specialty[], 70, '+92519290402');
